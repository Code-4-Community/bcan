...
const UserRowHeader = () => {
  ...
  function buttonHandler(header: keyof User) {
    const isAsc = labels.header == header ? (labels.sort == "asc" ? "desc" : labels.sort == "desc" ? "asc" : "none") : "desc";
  ...
  return (
    <div className="grid grid-cols-2 md:grid-cols-[30%_35%_25%_10%] cols border-b-2 border-grey-150 py-3 px-8 items-center">
      <UserRowHeaderButton header="Name" sort={labels.header == "lastName" ? labels.sort : "none"} onClick={() => buttonHandler("lastName")} />
      <UserRowHeaderButton header="Email" sort={labels.header == "email" ? labels.sort : "none"} onClick={() => buttonHandler("email")} />
      <UserRowHeaderButton header="Position" sort={labels.header == "position" ? labels.sort : "none"} onClick={() => buttonHandler("position")} />
