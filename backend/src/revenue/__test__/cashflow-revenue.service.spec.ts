...
const resolved = (value: unknown) => ({
  promise: vi.fn().mockResolvedValue(value),
});
const rejected = (error: unknown) => ({
  promise: vi.fn().mockRejectedValue(error),
});
const awsError = (code: string, message = "AWS error") => ({
  code,
  message,
...
const mockDatabase: CashflowRevenue[] = [
  {
    ...
    amount: 1000,
    type: RevenueType.Donation,
    installments: [{ amount: 1000, date: "2024-01-01" as any }],
  },
  {
    name: "Revenue Two",
    amount: 2000,
    type: RevenueType.Grants,
    installments: [{ amount: 2000, date: "2024-02-01" as any }],
  },
  {
    name: "Revenue Three",
    amount: 3000,
    type: RevenueType.Sponsorship,
    installments: [{ amount: 3000, date: "2024-03-01" as any }],
  },
];
...
describe("RevenueService", () => {
  ...
  describe("getAllRevenue", () => {
    ...
    it("should throw InternalServerErrorException when response is null", async () => {
      ...
      await expect(service.getAllRevenue()).rejects.toThrow(
        InternalServerErrorException,
      );
    });
    ...
    it("should throw InternalServerErrorException when table name env var is not set", async () => {
      ...
      await expect(serviceNoTable.getAllRevenue()).rejects.toThrow(
        InternalServerErrorException,
      );
    });
    ...
    it("should throw InternalServerErrorException on ResourceNotFoundException", async () => {
      mockScan.mockReturnValue(
        rejected(awsError("ResourceNotFoundException", "Table not found")),
      );
      await expect(service.getAllRevenue()).rejects.toThrow(
        InternalServerErrorException,
      );
    ...
    it("should throw InternalServerErrorException on ProvisionedThroughputExceededException", async () => {
      mockScan.mockReturnValue(
        rejected(awsError("ProvisionedThroughputExceededException")),
      );
      await expect(service.getAllRevenue()).rejects.toThrow(
        InternalServerErrorException,
      );
  ...
  describe("createRevenue", () => {
    ...
    it("should throw BadRequestException when item with same name already exists", async () => {
      ...
      await expect(service.createRevenue(mockRevenue)).rejects.toThrow(
        BadRequestException,
      );
    ...
    it("should throw BadRequestException on ConditionalCheckFailedException race condition", async () => {
      ...
      mockPut.mockReturnValue(
        rejected(
          awsError("ConditionalCheckFailedException", "Item already exists"),
        ),
      );
      await expect(service.createRevenue(mockRevenue)).rejects.toThrow(
        BadRequestException,
    ...
    it("should throw BadRequestException when revenue body is null", async () => {
      await expect(service.createRevenue(null as any)).rejects.toThrow(
        BadRequestException,
      );
    });
    ...
    it("should throw BadRequestException when type is invalid", async () => {
      await expect(
        service.createRevenue({
          ...mockRevenue,
          type: "InvalidType" as RevenueType,
        }),
      ).rejects.toThrow(BadRequestException);
    });
    ...
    it("should throw BadRequestException when type is null", async () => {
      await expect(
        service.createRevenue({ ...mockRevenue, type: null as any }),
      ).rejects.toThrow(BadRequestException);
    });

    it("should throw BadRequestException when type is undefined", async () => {
      await expect(
        service.createRevenue({ ...mockRevenue, type: undefined as any }),
      ).rejects.toThrow(BadRequestException);
    });
    ...
    it("should throw BadRequestException when name is null", async () => {
      await expect(
        service.createRevenue({ ...mockRevenue, name: null as any }),
      ).rejects.toThrow(BadRequestException);
    });

    it("should throw BadRequestException when name is undefined", async () => {
      await expect(
        service.createRevenue({ ...mockRevenue, name: undefined as any }),
      ).rejects.toThrow(BadRequestException);
    });

    it("should throw BadRequestException when name is an empty string", async () => {
      await expect(
        service.createRevenue({ ...mockRevenue, name: "" }),
      ).rejects.toThrow(BadRequestException);
    });
    ...
    it("should throw BadRequestException when installments is null", async () => {
      await expect(
        service.createRevenue({ ...mockRevenue, installments: null as any }),
      ).rejects.toThrow(BadRequestException);
    });
    ...
    it("should throw BadRequestException when installments is not an array", async () => {
      await expect(
        service.createRevenue({ ...mockRevenue, installments: "bad" as any }),
      ).rejects.toThrow(BadRequestException);
    });
    ...
    it("should throw InternalServerErrorException when table name is not set", async () => {
      ...
      const serviceNoTable = module.get<RevenueService>(RevenueService);
      await expect(serviceNoTable.createRevenue(mockRevenue)).rejects.toThrow(
        InternalServerErrorException,
      );
    });
  });
  ...
  describe("updateRevenue", () => {
    ...
    it("should trim both the route param and body name before comparing", async () => {
      mockGet.mockReturnValue(resolved({ Item: mockRevenue }));
      mockPut.mockReturnValue(resolved({}));
      const result = await service.updateRevenue("  Test Revenue  ", {
        ...mockRevenue,
        name: "  Test Revenue  ",
      });
    ...
    it("should not call delete when trimmed names are equal despite different surrounding whitespace", async () => {
      mockGet.mockReturnValue(resolved({ Item: mockRevenue }));
      mockPut.mockReturnValue(resolved({}));
      await service.updateRevenue("Test Revenue", {
        ...mockRevenue,
        name: "  Test Revenue  ",
      });
      expect(mockDelete).not.toHaveBeenCalled();
    ...
    it("should use the route param name as the DynamoDB get key, not the body name", async () => {
      mockGet.mockImplementation((params) => {
        if (params.Key.name === "Test Revenue 2") {
          return resolved({ Item: mockRevenue });
        }
        return resolved({});
      });
    ...
    it("should throw InternalServerErrorException when delete fails after successful put during rename", async () => {
      ...
      await expect(
        service.updateRevenue("Test Revenue", {
          ...mockRevenue,
          name: "New Name 3",
        }),
    ...
    it('should throw BadRequestException when renaming to a name that already exists', async () => {
  mockGet.mockImplementation((params) => {
    // Both the old name and the new name exist
    if (params.Key.name === 'Test Revenue' || params.Key.name === 'Existing Revenue') {
      return resolved({ Item: mockRevenue });
    }
    return resolved({});
  });
  ...
  await expect(
    service.updateRevenue('Test Revenue', { ...mockRevenue, name: 'Existing Revenue' })
  ).rejects.toThrow(BadRequestException);
  ...
  expect(mockDelete).not.toHaveBeenCalled();
    ...
    it("should throw BadRequestException when item does not exist", async () => {
      mockGet.mockReturnValue(resolved({ Item: undefined }));
      await expect(
        service.updateRevenue("Nonexistent", mockRevenue),
      ).rejects.toThrow(BadRequestException);
      expect(mockPut).not.toHaveBeenCalled();
    });
    ...
    it("should throw BadRequestException when revenue body is null", async () => {
      await expect(
        service.updateRevenue("Test Revenue", null as any),
      ).rejects.toThrow(BadRequestException);
    });
    ...
    it("should throw BadRequestException when route name is null", async () => {
      await expect(
        service.updateRevenue(null as any, mockRevenue),
      ).rejects.toThrow(BadRequestException);
    });

    it("should throw BadRequestException when route name is empty", async () => {
      await expect(service.updateRevenue("", mockRevenue)).rejects.toThrow(
        BadRequestException,
      );
    });
    ...
    it("should throw BadRequestException when amount is invalid", async () => {
      await expect(
        service.updateRevenue("Test Revenue", { ...mockRevenue, amount: -1 }),
      ).rejects.toThrow(BadRequestException);
    });

    it("should throw BadRequestException when amount is NaN", async () => {
      await expect(
        service.updateRevenue("Test Revenue", { ...mockRevenue, amount: NaN }),
      ).rejects.toThrow(BadRequestException);
    });
    ...
    it("should throw BadRequestException when body name is empty", async () => {
      await expect(
        service.updateRevenue("Test Revenue", { ...mockRevenue, name: "  " }),
      ).rejects.toThrow(BadRequestException);
    });
    ...
    it("should throw InternalServerErrorException when table name is not set", async () => {
      ...
      const serviceNoTable = module.get<RevenueService>(RevenueService);
      await expect(
        serviceNoTable.updateRevenue("Test Revenue", mockRevenue),
      ).rejects.toThrow(InternalServerErrorException);
    });
  });
  ...
  describe("deleteRevenue", () => {
    it("should delete a revenue item successfully", async () => {
      mockDelete.mockReturnValue(resolved({}));
      await expect(
        service.deleteRevenue("Test Revenue"),
    ...
    it("should throw BadRequestException when name is an empty string", async () => {
      await expect(service.deleteRevenue("")).rejects.toThrow(
        BadRequestException,
      );
    });
    ...
    it("should throw BadRequestException when name is null", async () => {
      await expect(service.deleteRevenue(null as any)).rejects.toThrow(
        BadRequestException,
      );
    });

    it("should throw BadRequestException when name is undefined", async () => {
      await expect(service.deleteRevenue(undefined as any)).rejects.toThrow(
        BadRequestException,
      );
    });
    ...
    it("should throw InternalServerErrorException when item does not exist (ConditionalCheckFailedException)", async () => {
      mockDelete.mockReturnValue(
        rejected(
          awsError("ConditionalCheckFailedException", "Item does not exist"),
        ),
      );
      await expect(service.deleteRevenue("Nonexistent")).rejects.toThrow(
        InternalServerErrorException,
      );
    });
    ...
    it("should throw InternalServerErrorException on ResourceNotFoundException", async () => {
      ...
      await expect(service.deleteRevenue("Test Revenue")).rejects.toThrow(
        InternalServerErrorException,
      );
    });
    ...
    it("should throw InternalServerErrorException when table name is not set", async () => {
      ...
      const serviceNoTable = module.get<RevenueService>(RevenueService);
      await expect(
        serviceNoTable.deleteRevenue("Test Revenue"),
      ).rejects.toThrow(InternalServerErrorException);
    });
  });
});
