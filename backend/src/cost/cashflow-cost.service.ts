...
export class CostService {
  ...
  private validateDate(date: string) {
    if (date === null) {
      throw new BadRequestException(
        "date is required and must be a non-null string",
      );
    }
    const iso8601Regex =
    ...
    if (!iso8601Regex.test(date)) {
      throw new BadRequestException(
        'date must be a valid ISO 8601 format string (e.g., "2026-03-22" or "2026-03-22T16:09:52Z")',
      );
    }
    ...
    try {
      ...
      if (!result.Item) {
        ...
        throw new NotFoundException(
          `Cost with name ${normalizedName} not found`,
        );
      }
      ...
      this.logger.error(
        `Failed to retrieve cost ${normalizedName}`,
        error as Error,
      );
    ...
    try {
      ...
      if (awsError.code === "ConditionalCheckFailedException") {
        throw new ConflictException(
          `Cost with name ${normalizedName} already exists`,
        );
      }
  ...
  async updateCost(
    costName: string,
    updates: CashflowCost,
  ): Promise<CashflowCost> {
    const tableName = process.env.CASHFLOW_COST_TABLE_NAME || "";
    this.validateName(costName);
    const normalizedName = costName.trim();
    ...
    if (isUnchanged) {
      this.logger.log(
        `No changes detected for cost ${normalizedName}; skipping update`,
      );
      return existingCost;
    }
    ...
    if (shouldRename) {
      this.logger.log(
        `Renaming cost ${normalizedName} to ${updates.name.trim()}`,
      );
      ...
      try {
        ...
        this.logger.log(
          `Successfully renamed cost ${normalizedName} to ${updates.name.trim()}`,
        );
        return updates;
      } catch (error) {
        if (error instanceof NotFoundException) {
        ...
        if (awsError.code === "TransactionCanceledException") {
          ...
          if (reasons[0]?.Code === "ConditionalCheckFailed") {
            this.logger.error(
    `Rename conflict: cost with name "${updates.name.trim()}" already exists, cannot rename from "${normalizedName}"`
  );
            throw new ConflictException(
              `Cost with name ${updates.name.trim()} already exists`,
            );
          }
          ...
          if (reasons[1]?.Code === "ConditionalCheckFailed") {
            this.logger.error(
    `Rename failed: original cost "${normalizedName}" no longer exists in the table`
  );
            throw new NotFoundException(
              `Cost with name ${normalizedName} not found`,
            );
    ...
    this.logger.log(
      `Replacing cost ${normalizedName} with payload: ${JSON.stringify(updates)}`,
    );
  ...
  async deleteCost(costName: string): Promise<string> {
    ...
    try {
      ...
      this.logger.error(
        `Failed to delete cost ${normalizedName}`,
        error as Error,
      );
      throw new InternalServerErrorException(
        `Failed to delete cost ${normalizedName}`,
