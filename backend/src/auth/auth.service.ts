...
async register(
  ...
  try {
    ...
    try {
      const createUserResponse = await this.cognito.adminCreateUser({
      ...
      if (!cognitoSub) {
        throw new InternalServerErrorException("Failed to retrieve user ID after creation");
      }

      cognitoUserCreated = true;
      this.logger.log(`✓ Cognito user created successfully for ${email}, sub: ${cognitoSub}`);

    } catch (cognitoError: any) {
      if (cognitoError instanceof HttpException) throw cognitoError;
      this.logger.error(`Cognito user creation failed for ${email}:`, cognitoError);
    ...
    try {
      ...
      this.logger.error(`Failed to set password for ${email}:`, passwordError);
    ...
    try {
      ...
      this.logger.error(`Failed to add ${email} to Inactive group:`, groupError);
      await this.rollbackCognitoUser(userPoolId, email);
    ...
    if (error instanceof Error) {
      ...
      this.logger.error(`Unknown error during registration for ${email}:`, error);
    }
...
private async rollbackCognitoUser(userPoolId: string, email: string): Promise<void> {
  ...
  try {
    ...
    this.logger.error(`Rollback failed: Could not delete Cognito user ${email}`, rollbackError);
    this.logger.error(`CRITICAL: User ${email} exists in Cognito but not in DynamoDB - manual cleanup required`);
  }
}
...
async login(
  ...
  try {
    ...
    this.logger.error(`Login failed for user ${email} with unknown error type`, error);
    throw new InternalServerErrorException("An unknown error occurred during login.");
  }
}
